...
async def release_rsrc_callback(osg_job):
...
async def trace_active_job():
    global terminate_job_count, completed_job_count, overload
    while True:
        if not running_job_tree.is_empty():
            end_date, osg_job = running_job_tree.min_item()
            now = datetime.now().timestamp()
            if osg_job['JobDuration'] <= scale_ratio*(now - osg_job['JobSimLastStartDate']):
                print(osg_job['JobDuration']-scale_ratio*(now - osg_job['JobSimLastStartDate']))
                running_job_tree.remove(end_date)
                await release_rsrc_callback(osg_job)
...
if __name__ == '__main__':
    ...
    thread3 = threading.Thread(name='process_jobs', target=process_job, daemon=True)
    thread1.start()
    thread2.start()
    thread3.start()
    # trace_active_job(release_rsrc_callback)
    loop = asyncio.get_event_loop()
    loop.run_until_complete(trace_active_job())